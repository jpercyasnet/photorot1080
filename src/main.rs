...
use iced::widget::{Column, Container, text, column, button, row, progress_bar};
...
//    PhotoRot1080::run(Settings::default())
}
...
impl Application for PhotoRot1080 {
    ...
    fn update(&mut self, message: Message) -> Command<Message>  {
        match message {
            ...
            Message::IRS(IrsMessage::StartButton) => {
                if Path::new(&self.dir_value).exists() {
                    stdCommand::new("indivrotate01")
                             .arg(&self.dir_value)
                             .spawn()
                             .expect("failed to execute process");
                    self.msg_value = "started indivrotate01 program".to_string();
                    self.mess_color = Color::from([0.0, 1.0, 0.0]);
                } else {
...
//            Message::IRS(message) => {self.irs_tab.update(message);Command::none()}
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        ...
        Tabs::new(self.active_tab, Message::TabSelected)
            ...
            .push(self.irs_tab.tab_label(), self.irs_tab.view())
